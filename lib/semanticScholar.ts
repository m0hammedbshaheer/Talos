// DOCUMENTATION NOTE: "Cascade contamination" means your paper cites a normal-looking
// source that itself cites a retracted paper—so bad science can spread indirectly.
// RetractWatch pulls each reference's own reference list (one hop) to surface that risk.
// Few free tools expose this second-level check.
...
/** One reference row from Semantic Scholar (only entries with a DOI are returned). */
  ...
  try {
    ...
    for (const item of rows) {
      try {
        ...
        // DEMO MOMENT: each resolved reference DOI is a node the graph can mark orange
        // when that downstream paper is retracted or risky.
        if (doiVal) {
